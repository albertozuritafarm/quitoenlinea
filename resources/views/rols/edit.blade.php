            ...
            <div class="col-md-12">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="nameRol" name="nameRol" placeholder="Nombre del Rol" value="{{$rol->name}}">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="rol_entity" id="rol_entity">
                            ...
                            @foreach($rolEntity as $r)
                            <option  @if($r->id == $rol->rol_entity_id) selected="true" @endif value="{{$r->id}}">{{$r->name}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="rol_type" id="rol_type">
                            <option value="">-- Escoja Una--</option>
                            @foreach($rolType as $r)
                            <option @if($r->id == $rol->rol_type_id) selected="true" @endif value="{{$r->id}}">{{$r->name}}</option>
                            @endforeach
                        </select>                    
                    </div>
                </div>
                    ...
                    <table id="newPaginatedTableNoOrdering" class="table table-striped row-border table-responsive hover stripe borderTable">
                        ...
                        <tbody>
                             ...
                             <tr style="background-color: #44444496;">
                                ...
                                <td align="center"><input type="checkbox" id="main_view_{{$main->id}}" class="chk" name="main_view_{{$main->id}}" value="{{$main->id}}_view" @if($main->sub_menu == 0) onchange="chkChange('main','view',{{$main->id}},'false')" @else onchange="chkChange('main','view',{{$main->id}},'true')" @endif @if($main->checkedView != null) checked @endif></td>
                                <td align="center"><input type="checkbox" id="main_edit_{{$main->id}}" class="chk" name="main_view_{{$main->id}}" value="{{$main->id}}_edit" @if($main->sub_menu == 0) onchange="chkChange('main','edit',{{$main->id}},'false')" @else onchange="chkChange('main','edit',{{$main->id}},'true')" @endif  @if($main->checkedEdit != null) checked @endif @if((checkPermits($main->id, 'edit')) == false) disabled="disabled" @endif></td>
                                <td align="center"><input type="checkbox" id="main_cancel_{{$main->id}}" class="chk" name="main_view_{{$main->id}}" value="{{$main->id}}_cancel" @if($main->sub_menu == 0) onchange="chkChange('main','cancel',{{$main->id}},'false')" @else onchange="chkChange('main','cancel',{{$main->id}},'true')" @endif  @if($main->checkedCancel != null) checked @endif @if((checkPermits($main->id, 'cancel')) == false) disabled="disabled" @endif></td>
                                <td align="center"><input type="checkbox" id="main_create_{{$main->id}}" class="chk" name="main_view_{{$main->id}}" value="{{$main->id}}_create" @if($main->sub_menu == 0) onchange="chkChange('main','create',{{$main->id}},'false')" @else onchange="chkChange('main','create',{{$main->id}},'true')" @endif  @if($main->checkedCreate != null) checked @endif @if((checkPermits($main->id, 'create')) == false) disabled="disabled" @endif></td>
                             </tr>
                                    ...
                                    @if($secondary->parent_id == $main->id)
                                        ...
                                        @if($third->parent_id == $secondary->id)
                                            <tr>
                                                ...
                                                <td align="center"><input type="checkbox" id="third_view_{{$main->id}}_{{$third->id}}"  class="chk child_third_view_{{$secondary->id}}" name="third_view_{{$main->id}}[]" value="{{$third->id}}_view" onchange="chkThirdChange('secondary','view',{{$third->id}},{{$main->id}},{{$secondary->id}})" @if($third->checkedView != null) checked @endif></td>
                                                <td align="center"><input type="checkbox" id="third_edit_{{$main->id}}_{{$third->id}}"  class="chk child_third_edit_{{$secondary->id}}" name="third_edit_{{$main->id}}[]" value="{{$third->id}}_edit" onchange="chkThirdChange('secondary','edit',{{$third->id}},{{$main->id}},{{$secondary->id}})" @if($third->checkedEdit != null) checked @endif @if((checkPermits($third->id, 'edit')) == false) disabled="disabled" @endif></td>
                                                <td align="center"><input type="checkbox" id="third_cancel_{{$main->id}}_{{$third->id}}"  class="chk child_third_cancel_{{$secondary->id}}" name="third_cancel_{{$main->id}}[]" value="{{$third->id}}_cancel" onchange="chkThirdChange('secondary','cancel',{{$third->id}},{{$main->id}},{{$secondary->id}})" @if($third->checkedCancel != null) checked @endif @if((checkPermits($third->id, 'cancel')) == false) disabled="disabled" @endif></td>
                                                <td align="center"><input type="checkbox" id="third_create_{{$main->id}}_{{$third->id}}"  class="chk child_third_create_{{$secondary->id}}" name="third_create_{{$main->id}}[]" value="{{$third->id}}_create" onchange="chkThirdChange('secondary','create',{{$third->id}},{{$main->id}},{{$secondary->id}})" @if($third->checkedCreate != null) checked @endif @if((checkPermits($third->id, 'create')) == false) disabled="disabled" @endif></td>
                                            </tr>
                                        @else
                                        @endif
                                        @endforeach
