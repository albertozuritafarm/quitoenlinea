...
class inspectionRequestR4UserEmail extends Mailable {
     ...
     * @return void
     */
    public function __construct($saleId,$customerEmail,$customerDocument,$inspectionId) {
        $this->saleId = $saleId;
    ...
    public function build() {
        $customerQuery = 'select concat(cus.first_name," ",IFNULL(cus.second_name, "")," ",cus.last_name," ",cus.second_last_name) as "Cliente", concat(cus.first_name," ",IFNULL(cus.second_name, "")) as "names", concat(cus.last_name," ",cus.second_last_name) as "lastNames", pro.ramodes as "ramo", cus.document as "cusDocument", DATE_FORMAT(sal.emission_date, "%Y") as "year", sal.id as "id", pro.name as "proName", DATE_FORMAT(sal.emission_date, "%d/%m/%Y") as "date", cus.email as "email", cus.mobile_phone as "mobile", cus.phone as "phone", sal.agen_id as "agen_id", CONCAT(users.first_name," ",users.last_name) as "user", ch.canalnegodes as "channel_name", doc.name as "typeDocDes", cus.birthdate as "cusBirthdate"
                from customers cus
        ...
        $propertieQuery = 'select  concat(main_street,", ",secondary_street,", ",number,", ",IFNULL(office_department,""),", ",city_id) AS "adress" 
                                    from properties 
                                    where sales_id = "' . $this->saleId . '"';
        $propertie = DB::select($propertieQuery);

        $inspectorQuery='SELECT concat(first_name," ",last_name) as "inspName" 
                                    FROM users
                                    where role_id="45"';
        $inspector = DB::select($inspectorQuery);
...
//        $sale = $this->saleId;
        ...
        return $this->view('emails.inspectionRequestR4User')
                    ->from($email_SS->value, $tag_SS->value)
                    ->subject('Notificación de Inspección realizada Cotización '.$this->saleId)
