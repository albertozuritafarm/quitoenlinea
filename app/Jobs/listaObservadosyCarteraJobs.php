...
class listaObservadosyCarteraJobs implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($carteraVencida === '500' || $listaObservados === '500'){
            ...
            if($carteraVencida == true || $listaObservados == true){
                ...
                $job = (new \App\Jobs\infoListsUserEmailJobs($this->saleId, $this->email, $customer->document));
                dispatch($job);
                ...
                infoListsEjecutivoSSEmailJobs::dispatch($this->saleId, $email[0]->ejecutivo_ss_email, $customer->document, $motive);
            }else{
                $salesUpdate = \App\sales::find($this->saleId);
