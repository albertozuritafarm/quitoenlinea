...
class listaObservadosyCarteraLegalPersonJobs implements ShouldQueue
{
    ...
    {
        set_time_limit(3000);
        $customer = \App\customerLegalRepresentative::find($this->customerId);
        $motive = '';
        ...
        if($carteraVencida === '500' || $listaObservados === '500'){
            ...
            if($carteraVencida ==true|| $listaObservados ==true){
                ...
                $customerUpdate = \App\customerLegalRepresentative::find($this->customerId);
        
                $customerUpdate->status_id = 24;                                                                                
                $customerUpdate->save();            

                $job = (new \App\Jobs\infoListsUserEmailMassivesVinculationLegalPersonJobs($this->saleId, $this->email, $customer->document));
                dispatch($job);
                ...
                infoListsEjecutivoSSEmailMassivesVinculationLegalPersonJobs::dispatch($this->saleId, $email[0]->ejecutivo_ss_email, $customer->document, $motive);
                return 'false';
            }else{
                $salesUpdate = \App\sales::find($this->saleId);
