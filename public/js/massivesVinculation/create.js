...
function BtnSave() {
    ...
    var second_last_name_validation=document.getElementById("second_last_name_validation");
    if(second_last_name.value !== ''){
        if (second_last_name.value.length > 30) {
...
//        console.log(emailValidate);
    ...
    var url = ROUTE + '/massivesVinculation/store';
    var data={documentNumber:documentNumber,first_name:first_name,second_name:second_name,document_id:document_id,last_name:last_name,second_last_name:second_last_name,mobile_phone:mobile_phone,email:email,branch:branch,emissionType:emissionType,insuredValue :insuredValue,netPremium:netPremium,product:product};
    $.ajax({
        type: "POST",
        data: {_token: CSRF_TOKEN,data},
        url: url,
