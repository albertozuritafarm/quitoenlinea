...
$(document).ready(function () {$(document).on('click', '.pagination a', function (event) {
    ...
    function fetch_data(page)
    {
        ...
        $.ajax({
            ...
            url: ROUTE + "/massives/secondary/fetch_data?page=" + page,
            beforeSend: function () {
                // Show Loader
...
//    });
      $('#btnClearMassive').click(function () {
        ...
        document.getElementById("cusDoc").value = "";
    });
});

function payMassive(id) {
    var r = confirm("¿Desea realizar el pago?");
    if (r == true) {
        var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
...
function salesResumeTable(id){
    ...
    $.ajax({
        ...
        success: function (data) {
            var modalBodySaleResume = document.getElementById("modalBodySaleResume");
            modalBodySaleResume.innerHTML = data;
            document.getElementById("modalBtnClickResume").click();
        }
    });
}

function cancelMassives(){
    var ids = new Array();
    $("input:checkbox[name=vehicle1]:checked").each(function(){
        ids.push($(this).val()); 
    });
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var data = {ids: ids};
    var url = ROUTE + '/massive/cancel/ajax';
    $.ajax({
        url: url,
        type: "POST",
        /* send the csrf-token and the input to the controller */
