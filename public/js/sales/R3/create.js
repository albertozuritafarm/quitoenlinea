...
function secondStepBtnNext() {
    ...
    var customerAlert1 = document.getElementById("customerAlert1");
    if (validate === 'true') {
        customerAlert1.classList.remove("hidden");
        $(customerAlert1).addClass("visible");
        return false;
    }else{
        customerAlert1.classList.remove("visible");
        $(customerAlert1).addClass("hidden");
    }

    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var url = ROUTE + '/sales/R3/check/price';
    $.ajax({
        type: "POST",
...
function thirdStepBtnNext(canalPlanId, today, oneYear, agenciaSS, ramo, prima, contribucion, sCam, derEmision, subSinIva, subConIva, iva, total, proId) {
    ...
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var data = {canalPlanId: canalPlanId, today: today, oneYear: oneYear, agenciaSS: agenciaSS, ramo: ramo, prima:prima, contribucion:contribucion, sCam:sCam, derEmision:derEmision, subSinIva:subSinIva, subConIva:subConIva, iva:iva, total:total, proId: proId};
    var url = ROUTE + '/sales/R3/resume/new/ss';
    $.ajax({
        type: "POST",
        data: {_token: CSRF_TOKEN, data},
        url: url,
        ...
        success: function (data) {
            var div = document.getElementById('R3TableBodyResume');
            div.innerHTML = '';
...
function executeSale() {
    ...
    //Obtain Sale Value
    var sBanco = document.getElementById("sBancos").value;
    ...
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var data = {rate:rate, total:total, tax:tax, subtotal12:subtotal12, subtotal0:subtotal0, dEmision:dEmision, sCampesino:sCampesino, sBanco:sBanco, product: selectedProduct, customer: customerData, saleMovement: saleMovement, saleId: saleId, sendQuotation: sendQuotation, occupation:occupation};
    var url = ROUTE + '/sales/R3/store';
    $.ajax({
        type: "POST",
        data: {_token: CSRF_TOKEN, data},
        url: url,
...
function openProductModal(id, prima){
    event.preventDefault();
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var url = ROUTE + '/sales/R2/check/conditions';
    $.ajax({
        type: "POST",
