...
//              $("#loaderBody").addClass('loaderBody');
            ...
            success: function (data) {
                if (data.success == 'true') {
                    ...
                    //if (data['birthdate'] == ''){ document.getElementById("birthdate").disabled = false; } else {document.getElementById("birthdate").value = data['birthdate'];}
...
//        console.log(emailValidate);
    ...
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var url = ROUTE + '/sales/R2/check/price';
    $.ajax({
        type: "POST",
...
function secondStepBtnNext(canalPlanId, today, oneYear, agenciaSS, ramo, prima, contribucion, sCam, derEmision, subSinIva, subConIva, iva, total, proId) {
    ...
    var url = ROUTE + '/sales/R2/resume/new/ss';
    $.ajax({
        type: "POST",
        data: {_token: CSRF_TOKEN, data},
        url: url,
        ...
        success: function (data) {
            var div = document.getElementById('R2TableBodyResume');
            div.innerHTML = '';
...
function executeSale() {
    ...
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var data = {rate:rate, total:total, tax:tax, subtotal0:subtotal0, subtotal12:subtotal12, dEmision:dEmision, sCampesino:sCampesino, sBanco:sBanco, product: selectedProduct, customer: customerData, saleMovement: saleMovement, saleId: saleId, sendQuotation: sendQuotation};
    var url = ROUTE + '/sales/R2/store';
    $.ajax({
        type: "POST",
        data: {_token: CSRF_TOKEN, data},
        url: url,
