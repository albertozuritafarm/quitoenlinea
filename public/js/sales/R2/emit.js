...
function validateSecondStep() {
    ...
    if (beginDate.value === '') {
        validate = true;
        $(beginDate).addClass('inputRedFocus');
    }
    var endDate = document.getElementById("endDate");
    if (endDate.value === '') {
        validate = true;
        $(endDate).addClass('inputRedFocus');
    }
    if ($('#uploaded_image').is(':empty')) {
        validate = true;
        var message = document.getElementById("message");
        $(message).css('display', 'block');
        $(message).html('Debe ingresar una imagen');
    ...
    if (validate === false) {
        var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
        var saleId = document.getElementById("saleId");
        var url = ROUTE + '/sales/emit/r2/store';
        $.ajax({
            url: url,
            type: "POST",
            /* send the csrf-token and the input to the controller */
            data: {_token: CSRF_TOKEN, beginDate: beginDate.value, endDate: endDate.value, saleId: saleId.value},
